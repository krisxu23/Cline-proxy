...
本仓库有大量「对齐上游/参考实现」的代码。**禁止凭记忆或推测写这类代码** —— 先查表：

| 要改的东西 | 先读 | 里面有什么 |
|---|---|---|
| zen / opencode 相关 | `docs/opencode-zen-facts.md` | 官方端点矩阵（**4 种端点**）、7 个免费模型及其端点与隐私条款、额度/风控的官方说法 vs 实测、**本项目缺失的能力**、以及一份「尚未确认」清单 |
| 客户端兼容性 | `docs/CLIENT-COMPAT.md` | 各客户端的怪癖与网关侧保障，每条都有锁定测试 |
| 与 OmniRoute 的对应关系 | `docs/omniroute-mapping.md` | 逐文件映射：指向参考实现，或标注「OmniRoute 无对应」（自研） |

**规则：**

1. 事实表里标 `[未知]` 的条目**禁止据此写代码** —— 先做实验，把结论回填表格并把标记改成
   `[实测]`（附日期）。
2. 事实表里已有结论的，**不要再重新推断**，直接引用。
3. 改了事实表覆盖的行为，**同一次提交里更新事实表**。
4. 不要照抄其他开源项目的同类实现 —— 那些项目可能基于**已失效**的假设
   （例：`docs/opencode-zen-facts.md` 第 7 节记录了 `Tlrenhb/opencode-free-gateway`
   用随机 UUID 做 session，正是被实测判定为 403 的形态）。

## 3. 构建

```bash
# go.mod 声明 go 1.26(与 CI 的 go-version 一致); 本机 PATH 实测为 1.27.0, 向下兼容构建。
...
入口包在 `cmd/free-router/`（标准布局）；`internal/` 下全部为库包，
业务代码只在 `internal/` 内演进。
...
CGO_ENABLED=0 go build -tags with_quic,with_grpc,with_utls \
  -ldflags="-s -w -H=windowsgui" -o "D:/目标/free-router.exe" ./cmd/free-router
```

windowsgui 下没有控制台，运行日志看 `data/free-router.log` 或管理面板。
