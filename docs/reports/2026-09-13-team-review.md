...
## 9. 上线后首个实测回退: "程序打不开了, 打开显示令牌提示页"(2026-09-13 21:05)

用户部署 §8 的版本后反馈程序打不开。排查结论: **程序在正常运行**(日志里模型同步、
订阅抓取、脱敏 URL 全在工作), 撞上的是 §7.5-3 与 Cookie 设计的叠加回退:

| 根因 | 说明 | 修复 |
|---|---|---|
| 会话 Cookie 无 Max-Age | 浏览器关一次就失效, 重开浏览器直接访问 `/admin/` 必然无令牌 | `adminCookieMaxAge = 180天`, 两处 SetCookie 都带上; Cookie 值即令牌, 令牌换则旧 Cookie 自然失效, 暴露面不变 |
| 无令牌=纯提示页, 无下一步 | §7.5-3 的安全收口没有给用户可操作的入口 | 引导页补令牌输入框(粘贴 `data/admin-token` 内容即进), 并写明"程序在正常运行, 双击 exe 弹窗/托盘入口已自带令牌" |

冒烟(隔离实例): 无 token → 2071 字节引导页含输入框; 带 `?token=` → 127701 字节全壳 +
`Set-Cookie: admin_token=…; Max-Age=15552000; HttpOnly; SameSite=Strict`;
仅带 Cookie → 直接全壳。gofmt/vet 干净、两组 go test ok、渲染测试 104 条 ALL PASS。

过程踩坑(记给自己): 隔离冒烟脚本里部署步骤 `cp dist/…` 用了**相对路径**, 而 cwd 已
切到冒烟目录, 报"文件被占用"其实是文件根本不存在 —— 错误信息把自己的根因都报错了。
另外本会话沙箱会在工具调用结束时回收它启动的子进程(Start-Process 也会被回收,
WMI/CIM 拉起被安全策略拦截), 所以"把程序跑起来"这一步只能由用户双击完成。
