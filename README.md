...
无需配置，启动即启用（匿名 key `public`）。每 10 分钟自动同步官方模型列表；付费 zen 模型显式 400 拒绝。超限时按 opencode 官方算法做摘要压缩（尾部预算 → 锚定摘要模板 → 重组会话）。支持 http/https/socks5 代理池轮询出口（round_robin / random / fill）。后台 **opencode 免费模型** 页可调全部参数。
